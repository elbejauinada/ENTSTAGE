...
<div class="container mx-auto p-8">
    ...
    <form action="{{ route('students.update', $student->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" id="name" name="name" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value="{{ old('name', $student->name) }}" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="date" id="birthday" name="birthday" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value="{{ old('birthday', $student->birthday) }}" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <select id="major_id" name="major_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                ...
                @foreach($majors as $major)
                    <option value="{{ $major->id }}" {{ $major->id == old('major_id', $student->major_id) ? 'selected' : '' }}>
                        {{ $major->name }}
                    </option>
                @endforeach
            </select>
