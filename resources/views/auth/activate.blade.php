...
<section class=" min-h-screen border-4 border-gray-900 flex box-border justify-center items-center">
    <div class="bg-[#dfa674] border-4 border-gray-900 rounded-2xl flex flex-col md:flex-row max-w-3xl p-8 items-center shadow-lg">
        <div class="md:w-1/2 px-8">
            <h2 class="font-bold text-3xl text-[#002D74]">Activate Account</h2>
            <p class="text-sm mt-4 text-[#002D74]">Enter your email to receive an activation link.</p>

            <form method="POST" action="{{ route('activate.email') }}" class="flex flex-col gap-4 mt-6">
                @csrf

                <div>
                    <label for="email" class="block font-medium text-sm text-gray-700">{{ __('Email') }}</label>
                    <input id="email" class="p-2 mt-2 rounded-xl border w-full" type="email" name="email" :value="old('email')" required autofocus placeholder="Email" />
                    @error('email')
        ...
        <div class="md:w-1/2 hidden md:block">
            <img class="rounded-2xl max-h-[1600px]" src="https://images.unsplash.com/photo-1552010099-5dc86fcfaa38?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w0NzEyNjZ8MHwxfHNlYXJjaHwxfHxmcmVzaHxlbnwwfDF8fHwxNzEyMTU4MDk0fDA&ixlib=rb-4.0.3&q=80&w=1080" alt="activation form image">
        </div>
    </div>
</section>
